  ...
  return async function (req, res, next) {
    try {
      ...
      if (mode === "view" && !permission.isViewPermission) {
        return res.status(403).json({ success: false, status: 200, message: "You don't have permission to view" });
      }
      if (mode === "add" && !permission.isAddPermission) {
        return res.status(403).json({ success: false, status: 200, message: "You don't have permission to add" });
      }
      if (mode === "edit" && !permission.isEditPermission) {
        return res.status(403).json({ success: false, status: 200, message: "You don't have permission to edit" });
      }
      if (mode === "delete" && !permission.isDeletePermission) {
        return res.status(403).json({ success: false, status: 200, message: "You don't have permission to delete" });
      }
